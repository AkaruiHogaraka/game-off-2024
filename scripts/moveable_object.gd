...
func _physics_process(delta: float) -> void:
	...
	if not _is_on_floor():
		...
		if moveable_parent.test_move(moveable_parent.transform, Vector2.DOWN, collision_info): 
			if ground_ray.is_colliding():
				moveable_parent.global_position.y = ground_ray.get_collision_point().y
			return
...
func _is_on_floor() -> bool:
	if left_ray.is_colliding() or right_ray.is_colliding() or ground_ray.is_colliding(): return true
	else: return false
